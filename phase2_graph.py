...
# --- Configuration ---
# Update path to look in the current directory instead of parent
CSV_PATH = os.path.join(os.getcwd(), 'database.csv')  # Look in current working directory
OUTPUT_HTML_PATH = os.path.join(os.path.dirname(__file__), 'influencer_sponsor_graph.html')

# Add option to filter by minimum frequency
MIN_FREQUENCY = 1  # Only show relationships that occur at least this many times
# ---
...
def visualize_with_plotly(G, output_path):
    ...
    # --- Prepare Plotly data ---
    # Use a more spaced layout for better visualization
    pos = nx.spring_layout(G, k=1.0, iterations=100)  # Increased k and iterations for better spacing
    ...
    # Enhance hover information to show frequency
    edge_hover_text = []
    for edge in G.edges(data=True):
        edge_hover_text.append(f"Sponsor: {edge[0]}<br>Creator: {edge[1]}<br>Frequency: {edge[2]['frequency']}")
    
    # Add hover text to edge traces
    solid_edge_trace = go.Scatter(
        ...
        line=dict(width=2, color='#888'),  # Make solid lines thicker
        hoverinfo='text',
        hovertext=[text for i, text in enumerate(edge_hover_text) if edge_styles[i//3] == 'solid'],
        mode='lines')
