...
CSV_FILENAME = os.path.join(
    os.getcwd(),
    'database.csv')  # Save database.csv in the current working directory
BATCH_SIZE = 10
...
def initialize_csv(filename=CSV_FILENAME):
    ...
    print(f"Checking for file at: {os.path.abspath(filename)}"
          )  # Print the absolute path for debugging
    CSV_COLUMNS = [
        'youtube video link', 'transcript', 'Sponsor Name', 'Sponsor Industry',
    ...
    if not os.path.isfile(filename):
        ...
        print(f"{filename} already exists."
              )  # Added explicit message when file exists
...
def save_to_csv(video_link, transcript, gemini_output, filename=CSV_FILENAME):
    ...
    if gemini_output:
        ...
        for line in lines:
            if line.startswith("Sponsor Name:"):
                ...
                sponsor_industry = line.replace("Sponsor Industry:",
                                                "").strip()
            elif line.startswith("Video Creator Name:"):
                video_creator_name = line.replace("Video Creator Name:",
                                                  "").strip()
            elif line.startswith("Video Creator Industry:"):
                video_creator_industry = line.replace(
                    "Video Creator Industry:", "").strip()
            elif line.startswith("Sponsor Video Link (if any):"):
                sponsor_website = line.replace("Sponsor Video Link (if any):",
                                               "").strip()
    ...
    data = {
        ...
        'Video Creator Name':
        [video_creator_name if video_creator_name else ''],
        'Video Creator Industry':
        [video_creator_industry if video_creator_industry else ''],
...
def get_channel_id_by_name(channel_name):
    ...
    if not YOUTUBE_API_KEY:
        print(
            "Error: YouTube API key is missing. Please add it to the configuration."
        )
        return None
    ...
    try:
        ...
        search_response = youtube.search().list(q=channel_name,
                                                type='channel',
                                                part='id,snippet',
...
def get_channel_videos(channel_id, max_results=50):
    ...
    try:
        ...
        channels_response = youtube.channels().list(part='contentDetails',
...
def channel_search_mode():
    ...
    try:
        max_videos = input(
            "Maximum number of videos to process (default 50): ").strip()
    ...
    # Confirm processing
    confirm = input(
        f"Process {len(video_urls)} videos? (y/n): ").strip().lower()
    if confirm != 'y':
    ...
    try:
        ...
        while len(videos) < max_results:
            search_response = youtube.search().list(
                ...
                videoDuration=
                'medium',  # Focus on medium-length videos (more likely to have sponsors)
                relevanceLanguage=
                'en'  # English videos for better transcript availability
            ).execute()
...
def industry_search_mode():
    ...
    industry = input(
        "Enter industry to search for (e.g., tech, beauty, fitness): ").strip(
        )
    if not industry:
    ...
    try:
        max_videos = input(
            "Maximum number of videos to search for (default 20): ").strip()
    ...
    # Search for videos
    print(
        f"Searching for up to {max_videos} videos related to '{industry}' with potential sponsors..."
    )
                ...
                if '-' in selection:
                    ...
                    for num in range(start, end + 1):
                        if 1 <= num <= len(video_results):
                            videos_to_process.append(video_results[num -
                                                                   1]['url'])
                else:
                    num = int(selection)
                    if 1 <= num <= len(video_results):
